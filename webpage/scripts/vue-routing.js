Vue.component('navigation-index', {template: '<nav><router-link to="/">Home</router-link> | <router-link to="/zeitrechner">Zeitrechner</router-link> | <router-link to="/zeitrechner2">Zeitrechner - Neu</router-link> | <router-link to="/midnightformula">Mitternachtsformel</router-link></nav>'});
...
Vue.component('timecalc2', { template: '<div><h2>Zeitrechner2</h2><form>	<input type="text" id="inputStr" style="width:80px;" placeholder="hh:mm"><br><br><button id="btnTimeCalc" class="btn btn-dark" onclick="timeByIdStr()">Calculate</button></form><div id="ausgabe"></div></div>'});
